...
// Function prototypes
void display_catalog(int sockfd, struct sockaddr_in server_addr);
void search_book(int sockfd, struct sockaddr_in server_addr);
void order_book(int sockfd, struct sockaddr_in server_addr);
void pay_for_book(int sockfd, struct sockaddr_in server_addr);
void send_request(int sockfd, struct sockaddr_in server_addr, char *message);
...
int main() {
    ...
    while (1) {
        ...
        switch (choice) {
            case 1:
                display_catalog(sockfd, server_addr);
                break;
            case 2:
                search_book(sockfd, server_addr);
                break;
            case 3:
                order_book(sockfd, server_addr);
                break;
            case 4:
                pay_for_book(sockfd, server_addr);
                break;
            case 5:
                close(sockfd);
                exit(0);
...
void display_catalog(int sockfd, struct sockaddr_in server_addr) {
    ...
    while (1) {
        ...
        int n = recvfrom(sockfd, buffer, MAX_MESSAGE_SIZE, 0, NULL, NULL);
        if (n == -1) {
...
void search_book(int sockfd, struct sockaddr_in server_addr) {
    ...
    char response[MAX_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    int n = recvfrom(sockfd, response, MAX_RESPONSE_SIZE, 0, NULL, NULL);
    if (n == -1) {
        perror("recvfrom failed");
        return;
    }
...
void send_request(int sockfd, struct sockaddr_in server_addr, char *message) {
    int send_len = sendto(sockfd, message, strlen(message), 0, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (send_len == -1) {
